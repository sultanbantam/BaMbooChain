...
import { History, Save, Calendar, Info, Camera } from 'lucide-react';

function PlantPastPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if(!pastDate || !type || !location || !fotoTanaman || !selfiePetani) {
      alert('Mohon lengkapi semua data riwayat penanaman beserta bukti foto!');
      return;
    }
    ...
    <div style={{ background: '#f8f9fa', minHeight: '100vh', paddingTop: '160px', paddingBottom: '80px' }}>
      ...
      <div className="container" style={{ maxWidth: '600px' }}>
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <h1 style={{ fontSize: '2rem', fontWeight: '800', color: 'var(--text-main)', marginBottom: '8px' }}>Catat Penanaman Lalu</h1>
          <p style={{ color: 'var(--text-muted)' }}>Masukkan data bambu yang sudah Anda tanam sebelumnya.</p>
        </div>

        <div style={{ background: 'white', padding: '32px', borderRadius: '24px', border: '1px solid #eee', boxShadow: '0 10px 30px rgba(0,0,0,0.03)' }}>
          <div style={{ background: '#e7f5ff', padding: '12px 16px', borderRadius: '12px', display: 'flex', gap: '10px', marginBottom: '24px', border: '1px solid #a5d8ff' }}>
             <Info size={18} color="#1c7ed6" style={{ flexShrink: 0 }} />
             <p style={{ fontSize: '0.85rem', color: '#1c7ed6', margin: 0 }}>
               Gunakan fitur ini jika Anda baru saja mendaftar tapi sudah memiliki tanaman bambu di lapangan.
             </p>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <div style={{ position: 'relative' }}>
                <input 
                  type="date" 
                  style={{ width: '100%', padding: '12px 16px', border: '1px solid #dee2e6', borderRadius: '12px', fontSize: '1rem', outline: 'none', color: 'var(--text-main)' }}
                  value={pastDate} 
            ...
            <div style={{ marginBottom: '32px' }}>
              ...
              >
                <option value="planted">Baru Ditanam (Perlu Pemeliharaan)</option>
                <option value="growing">Sedang Tumbuh (Siap Panen)</option>
              </select>
            </div>
            ...
            >
              {isSubmitting ? 'Menyimpan...' : <><Save size={20} /> Tambah Rekaman</>}
            </button>
          </form>
        </div>
      </div>
